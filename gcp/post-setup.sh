...
NOMAD_TOKEN=$(curl -s --header "Authorization: Bearer ${CONSUL_BOOTSTRAP_TOKEN}" "${LB_ADDRESS}:8500/v1/kv/nomad_mgmt_token?raw")
...
if [ ! -f $NOMAD_USER_TOKEN_FILENAME ]; then
    ...
    if [ ${#NOMAD_TOKEN} -eq 36 ]; then
        ...
        DELETE_TOKEN=$(curl -s -X DELETE --header "Authorization: Bearer ${CONSUL_BOOTSTRAP_TOKEN}" "${LB_ADDRESS}:8500/v1/kv/nomad_mgmt_token")
